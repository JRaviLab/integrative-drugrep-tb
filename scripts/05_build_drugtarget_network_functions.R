...
# library(readr)
# # get union of drugs from RA results
# RAresult_TB_microarray_df <- read_tsv("~/Documents/GitHub/drugrep_tb/results/uniformly_processed/microarray/04_rank_aggregation/RAresult_0.9pct0.5rv_indivSig_TB_microarray.tsv")
# RAresult_TB_RNAseq_df <- read_tsv("~/Documents/GitHub/drugrep_tb/results/uniformly_processed/RNAseq/04_rank_aggregation/RAresult_0.9pct0.5rv_indivSig_TB_RNAseq.tsv")
# RAresult_TB_union = merge(RAresult_TB_microarray_df, RAresult_TB_RNAseq_df, by="drug_name", all.x = TRUE, all.y = TRUE)
# union_RAresult_drugs = unique(RAresult_TB_union$drug_name)
# union_RAresult_drugs_df = as.data.frame(union_RAresult_drugs)
# #write_tsv(union_RAresult_drugs_df, file = "~/Documents/GitHub/drugrep_tb/results/uniformly_processed/union_RAresult_drugs.tsv")
#
# # get aggregated TB genes for disease genes
# # only do genes that came up in both microarray and RNAseq
# # upregulated genes
# marray_upaggr = read_tsv("./data/uniformly_processed/microarray/signatures/up/up_aggregated_signature.tsv")
# marray_upaggr_genes = marray_upaggr$GeneID
#
# rnaseq_upaggr = read_tsv("./data/uniformly_processed/RNAseq/signatures/up/up_aggregated_signature.tsv")
# rnaseq_upaggr_genes = rnaseq_upaggr$GeneID
#
# upaggr_genes = intersect(marray_upaggr_genes, rnaseq_upaggr_genes)
# upaggr_union_genes = union(marray_upaggr_genes, rnaseq_upaggr_genes)
#
# # downregulated genes
# marray_dnaggr = read_tsv("./data/uniformly_processed/microarray/signatures/dn/dn_aggregated_signature.tsv")
# marray_dnaggr_genes = marray_dnaggr$GeneID
#
# rnaseq_dnaggr = read_tsv("./data/uniformly_processed/RNAseq/signatures/dn/dn_aggregated_signature.tsv")
# rnaseq_dnaggr_genes = rnaseq_dnaggr$GeneID
#
# dnaggr_genes = intersect(marray_dnaggr_genes, rnaseq_dnaggr_genes)
# dnaggr_union_genes = union(marray_dnaggr_genes, rnaseq_dnaggr_genes)
#
# # combine up and down genes into one vector
# diffexp_genes = c(upaggr_union_genes, dnaggr_union_genes)
# # convert gene IDs to gene symbols
# # read in conversion table
# Homo_sapiens_gene_info <- read_csv("./data/metadata/Homo_sapiens.gene_info.csv")
# TBdiffexp_gene_table <- Homo_sapiens_gene_info[which(Homo_sapiens_gene_info$GeneID %in% diffexp_genes),]
# TBdiffexp_gene_symbols <- TBdiffexp_gene_table$Symbol
# #write_tsv(TBdiffexp_gene_table, file = "./results/uniformly_processed/shared_TBaggr_genes.tsv")
# #-----------------------------------------------------------------------------------------------------
# # get top disease pathways for each cluster? -- too few
# # only do pathways that came up in both microarray and RNAseq
# # upregulated genes --> pathways
# #####-----------------------------------------------------------------------
# # grab full disease pathways instead "marray_union_GO_up.tsv"
# mup_pathways = read_tsv("./data/pathways/microarray/up/GO_ORA/marray_union_GO_up.tsv")$Description
# rup_pathways = read_tsv("./data/pathways/RNAseq/up/GO_ORA/rnaseq_union_GO_up.tsv")$Description
# TB_up_pathways = union(mup_pathways,rup_pathways)
#
# mdn_pathways = read_tsv("./data/pathways/microarray/dn/GO_ORA/marray_union_GO_dn.tsv")$Description
# rdn_pathways = read_tsv("./data/pathways/RNAseq/dn/GO_ORA/rnaseq_union_GO_dn.tsv")$Description
# TB_dn_pathways = union(mdn_pathways,rdn_pathways)
#
# TB_pathways = union(TB_up_pathways,TB_dn_pathways)
# TB_drug_shared_pw = c("Demethylation","Adenylate cyclase-activating adrenergic receptor signaling pathway")
# #---------------------------------------------------------------------------
# # grab top drug-perturbed genes from union_RAresult_drugs
# signature_matrix = read_tsv("./data/drug_data/GSE92742_level5_signatures_lm.tsv")
# signature_metadata = read_tsv("./data/metadata/GSE92742_Broad_LINCS_sig_info.txt")
#
# drug_genes_df = get_drug_perturbed_genes(drug_names = union_RAresult_drugs,  signature_matrix, signature_metadata, time_point = "24 h", dosage = "10 µM", N = 5)
# #----------------------------------------------------------------------------------------------------
# # get node and edge tables for disease-drug target network
# #** fix node degrees of the subnetwork based on disease-drug shared pathways
# union_drug_tgpwmoas_demethHM = get_drugtarget_NetworkNodeData(drug_names = union_RAresult_drugs,
#                                                               drug_genes_df,
#                                                               moas = c("HMGCR inhibitor"),
#                                                               disease_pathways = c("demethylation"))
# union_nodes_demethHM = add_STRING_genes(union_drug_tgpwmoas_demethHM, disease_genes = TBdiffexp_gene_symbols)
# union_edges_demethHM = get_drugtarget_NetworkEdgeData(union_nodes_demethHM)
#
# write_tsv(union_nodes_demethHM,file = "./results/uniformly_processed/DTnodes_RAunion_druggenes_deg4_demeth_hmgcr.tsv")
# write_tsv(union_edges_demethHM,file = "./results/uniformly_processed/DTedges_RAunion_druggenes_deg4_demeth_hmgcr.tsv")
#
# #-----------------
# union_drug_tgpwmoas_adenyl = get_drugtarget_NetworkNodeData(drug_names = union_RAresult_drugs,
#                                                             drug_genes_df,
#                                                             moas = c(),
#                                                             disease_pathways = c("adenylate cyclase-activating adrenergic receptor signaling pathway"))
# union_nodes_adenyl = add_STRING_genes(union_drug_tgpwmoas_adenyl, disease_genes = TBdiffexp_gene_symbols)
# union_edges_adenyl = get_drugtarget_NetworkEdgeData(union_nodes_adenyl)
#
# write_tsv(union_nodes_adenyl,file = "./results/uniformly_processed/DTnodes_RAunion_druggenes_deg4_adenyl.tsv")
# write_tsv(union_edges_adenyl,file = "./results/uniformly_processed/DTedges_RAunion_druggenes_deg4_adenyl.tsv")
#
# #---------------------------------------------------------------------------------------------------
#
