...
bg_from_data <- function(metadata_path, data_path, extension = "") {
  ...
  for (i in 1:nrow(data_to_run)) {
    ...
    if (file.exists(file_path)) {
      ...
      dataset <- na.omit(dataset) # remove rows with all NAs

      if ("GeneID" %in% colnames(dataset)) {
        union_bg_data_genes <- c(union_bg_data_genes, as.character(dataset$GeneID))
      } else {
        warning(paste("No 'GeneID' column in file:", file_path))
      }
    } else {
