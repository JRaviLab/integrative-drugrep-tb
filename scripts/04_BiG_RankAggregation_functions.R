# functions to perform rank aggregation using BiG approaches obtained from the author of Li et. al., 2018
# ref1: https://github.com/xuelilyli/BiG
# ref2: https://github.com/baillielab/comparison_of_RA_methods/blob/main/algorithms/BiG_code_platform_changed.R
...
#'
init_W <- function(r) {
  W <- base::matrix(NA, base::dim(r)[1], base::dim(r)[2])
  rownames(W) <- 1:(base::dim(r)[1])
  for (i in 1:base::dim(r)[2]) {
    W[base::order(r[, i], na.last = NA), i] <- base::sort(stats::rnorm(base::sum(!is.na(r[, i]))), decreasing = TRUE)
  }
  return(W)
}
...
#' set.seed(1234)
#' sim <- sim_lvm(
#'   G = 25, S = 6, n_p1 = 3, rho = runif(6, min = 0.3, max = 0.9), p_p1 = 0.6, p_p2 = 0.8,
...
BiG_diffuse <- function(r, n_T, n_p1 = "none", M = 20000, burnin, prior, ds = 1, dp = 1, W = init_W(r), sigma_p10 = 0.5, sigma_p20 = 0.5, mu0 = numeric(G),
  ...
  for (m in 2:M)
  {
    ...
    {
      if (prior == "IG") {
        Sigma_s2[s, m] <- 1 / stats::rgamma(1,
          shape = sum(is.na(W[, s]) == F) / 2 + ds,
          rate = sum((W[, s] - Mu[, m] - Kappa1[, m])^2, na.rm = TRUE) / 2 + ds
        )
      } else if (prior == "uniform") {
        Sigma_s2[s, m] <- 1 / rtruncgamma(1,
          a = a, b = b, shape = (sum(is.na(W[, s]) == F) - 1) / 2,
    ...
    if (prior == "IG") {
      Sigma_p12[m] <- 1 / stats::rgamma(1, shape = length(P1) / 2 + dp, rate = sum(Kappa1[P1, m]^2, na.rm = TRUE) / 2 + dp)
    }
    # Sigma_p22[m]=1/stats::rgamma(1,shape=length(P2)/2+dp,rate=sum(Kappa2[P2,m]^2,na.rm=TRUE)/2+dp)}
    else if (prior == "uniform") {
      Sigma_p12[m] <- 1 / rtruncgamma(1, a = a, b = b, shape = (length(P1) - 1) / 2, rate = sum(Kappa1[P1, m]^2, na.rm = TRUE) / 2)
    }
      ...
      for (s in 1:(S))
      {
        ...
        for (g in index)
        {
          if (g == 1) {
            ...
            aa <- W[which(r[, s] == g + 1), s]
            bb <- W[which(r[, s] == g - 1), s]
          }
          W[which(r[, s] == g), s] <- truncnorm::rtruncnorm(1, a = aa, b = bb, mean = Mu[which(r[, s] == g), m] + Kappa1[which(r[, s] == g), m], sd = sqrt(Sigma_s2[s, m]))
        }
        if (n_T[s] < length(W[is.na(W[, s]) == F, s])) {
          W[which(r[, s] == (n_T[s] + 1)), s] <- truncnorm::rtruncnorm(sum(r[, s] == n_T[s] + 1, na.rm = TRUE),
            b = W[which(r[, s] == n_T[s]), s],
...
BiG_DA <- function(r, n_T, n_p1, M = 20000, burnin, a = 0.0202, b = 98.5025, dp = 1, W = init_W(r), sigma_p10 = 0.5, sigma_p20 = 0.5, mu0 = numeric(G),
  ...
  for (m in 2:M)
  {
    ...
    # update sigma_p2
    Sigma_p12[m] <- 1 / stats::rgamma(1,
      shape = length(P1) / 2 + 1,
      rate = sum(xi1[P1, m]^2, na.rm = TRUE) / 2 + 1
    )
    Sigma_p22[m] <- 1 / stats::rgamma(1,
      shape = length(P2) / 2 + 1,
